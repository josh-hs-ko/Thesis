...
module OptimisedPredicate {I J} {e : J → I} {D E} (O : Orn e D E) where
  ...
  μE-square : Square Fun l r
  μE-square = span l (sliceMorphism id frefl)
                     (sliceMorphism < e ** forget O , singleton ∘ forget O ∘ proj₂ >
                                    (λ { (j , y) → cong₂-pair refl (≡-to-≅ (forget-singOrn (singleton (forget O y)))) }))

  μE-med : (p : Square Fun l r) → SpanMorphism (SliceCategory Fun (Σ I (μ D))) l r p μE-square
  ...
  μE-med-unique : (p : Span (SliceCategory Fun (Σ I (μ D))) l r) →
                  Unique (Category.Morphism (SpanCategory (SliceCategory Fun (Σ I (μ D))) l r) p μE-square) (μE-med p)
  μE-med-unique p med' = fsym (SpanMorphism.triangle-l med')

  μE-pullback : Pullback Fun l r μE-square
  ...
  componentIso (ok j) =
    Setoid.sym (IsoSetoid Fun)
      (compIso wholeIso π₁ (forget (diffOrn-l O ⌈ singOrn D ⌉)) frefl
               ...
               (λ {j'} y →
                  decouple (cong proj₁ (SpanMorphism.triangle-l (proj₁ (OptP-pullback μE-square)) (j' , y))) refl)
               j)
