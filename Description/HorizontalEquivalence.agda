...
ṀHEq is xs ys = All-Ṁ (λ { _ (x , y) → x ≅ y}) is (Ṁ-comp is xs ys)

data HoriEq {I : Set} {X Y : I → Set} : (D : RDesc I) → ⟦ D ⟧ X → (E : RDesc I) → ⟦ E ⟧ Y → Set₁ where
  ṿ   : {is : List I} {xs : Ṁ X is} {ys : Ṁ Y is} → ṀHEq is xs ys → HoriEq (ṿ is) xs (ṿ is) ys
...
HoriEq-refl {I} {ṿ is } {X} {xs    } = ṿ (aux is xs)
 where
   aux : (is : List I) (xs : Ṁ X is) → ṀHEq is xs xs
   aux []       _        = tt
...
HoriEq-sym {I} {X = X} {Y} (ṿ heqs)  = ṿ (aux _ _ _ heqs)
  where
    aux : (is : List I) (xs : Ṁ X is) (ys : Ṁ Y is) → ṀHEq is xs ys → ṀHEq is ys xs
    aux []       _        _        _            = tt
...
HoriEq-trans {I} {X = X} {Y} {Z} (ṿ heqs)     (ṿ heqs')   = ṿ (aux _ _ _ _ heqs heqs')
  where
    aux : (is : List I) (xs : Ṁ X is) (ys : Ṁ Y is) (zs : Ṁ Z is) → ṀHEq is xs ys → ṀHEq is ys zs → ṀHEq is xs zs
    aux []       _        _        _        _            _              = tt
